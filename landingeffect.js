function draw(e) {
	var xCount = 35;
	var yCount = 80;
	var iXCount = 1 / (xCount - 1);
	var iYCount = 1 / (yCount - 1);
	var time = e * 0.001;
	var timeStep = 0.01;
	var grad = ctx.createLinearGradient(-width, 0, width, height);
	var t = time % 1;
	var tSide = floor(time % 2) === 0;
	var hueA = tSide ? 340 : 210;
	var hueB = !tSide ? 340 : 210;
	var colorA = hsl(hueA, 100, 50);
	var colorB = hsl(hueB, 100, 50);
	...
	for (var j = 0; j < yCount; j++) {
		var tj = j * iYCount;
		var c = cos(tj * TAU + time) * 0.1;
		for (var i = 0; i < xCount; i++) {
			var _t = i * iXCount;
			var n = noise.noise3D(_t, time, c);
			var y = n * height_half;
			var x = _t * (width + 20) - width_half - 10;
			(i ? lineTo : moveTo)(x, y);
		}
		time += timeStep;
	}
